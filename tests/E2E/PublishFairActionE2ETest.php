...
BASH;
        ...
        self::assertSame('did:plc:e2e-test-did', $outputs['did'] ?? null);
        ...
        self::assertSame('did:plc:e2e-test-did', $metadata['id'] ?? null);
    }
...
BASH;
        ...
        $result = ScriptRunner::run(
            ...
            [
                ...
                'FAIR_VERIFICATION_KEY_PRIVATE' => 'verification-private',
                'FAIR_VERIFICATION_KEY_PUBLIC' => 'verification-public',
                'FAIR_DID' => 'did:plc:e2e-fallback-did',
            ]
        );
        ...
        self::assertSame('did:plc:e2e-fallback-did', $outputs['did'] ?? null);
        self::assertArrayHasKey('metadata_path', $outputs);
    }
      ...
      public function testPublishActionBuildsAndUploadsMissingAssetFromWorkspace(): void
      {
        ...
        $initialized = ScriptRunner::run(
          ...
          [
            ...
            'FAIR_VERIFICATION_KEY_PRIVATE' => 'verification-private',
            'FAIR_VERIFICATION_KEY_PUBLIC' => 'verification-public',
            'FAIR_DID' => 'did:plc:e2e-build-did',
          ]
        );
    ...
    BASH;
        ...
        $outputs = GitHubOutputParser::parseFile($outputFile);
        self::assertSame('v1.2.3', $outputs['version'] ?? null);
        self::assertSame('did:plc:e2e-build-did', $outputs['did'] ?? null);
      }

    public function testPublishActionSucceedsWithVerificationKeysOnlyAndExistingDid(): void
    {
        $workspace = sys_get_temp_dir() . '/fair-e2e-workspace-' . bin2hex(random_bytes(5));
        mkdir($workspace, 0777, true);
...
BASH;
        ...
        $result = ScriptRunner::run(
            ...
            [
                ...
                'FAIR_VERIFICATION_KEY_PRIVATE' => 'verification-private',
                'FAIR_VERIFICATION_KEY_PUBLIC' => 'verification-public',
                'FAIR_DID' => 'did:plc:existing-test-did',
            ]
        );
        ...
        $outputs = GitHubOutputParser::parseFile($outputFile);
        self::assertSame('v1.2.3', $outputs['version'] ?? null);
        self::assertSame('did:plc:existing-test-did', $outputs['did'] ?? null);
        self::assertArrayHasKey('metadata_path', $outputs);
        ...
        $metadata = json_decode((string) file_get_contents($metadataPath), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('did:plc:existing-test-did', $metadata['id'] ?? null);

        self::assertStringContainsString('Using existing DID', $result->stdout);
        self::assertStringNotContainsString('Create DID', $result->stdout);
    }

    public function testPublishActionFailsWithNoDID(): void
    {
        $workspace = sys_get_temp_dir() . '/fair-e2e-workspace-' . bin2hex(random_bytes(5));
        mkdir($workspace, 0777, true);
